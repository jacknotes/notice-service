...
### 已实现
- **农历 cron 调度**：cron 触发方式支持中国农历（引入 lunar-go）——`@lunar monthly <日> <HH:MM>`（每月）、`@lunar yearly <月> <日> <HH:MM>`（每年，如正月初一）、`@lunar term <节气> <HH:MM>`（节气）；任务表单加农历快捷选择器；保存时语法预检；`cron_expr` 列加宽（迁移 018）
- **批量操作体系**：渠道 / 模板 / 任务 / 用户 / 分类五个管理页的「批量删除」按钮统一升级为「操作」下拉按钮（hover 展开）——渠道/模板/任务支持批量启用/禁用、批量变更分类（任务另支持批量变更投递渠道与接收地址）；用户支持批量禁用/启用/重置密码/强制开启/强制关闭双因子认证；分类支持批量删除未引用项；模板新增 enabled 字段与状态开关列（迁移 016 + 019 修正用户名唯一约束）
- **普通管理员可对 admin 强制开/关 2FA**：与删除/禁用/重置密码不同，2FA 是可恢复配置，允许普通管理员操作内置 admin（其它管理操作仍受保护）
- **CLI 重置密码连带清除 2FA**：`reset-password` 重置密码时同时清除该用户双因子认证（防「开 2FA 丢手机」后账号永久锁死）
- **删除用户后可重建同名**：username 唯一约束与软删除解耦（生成列 active_username 唯一索引，迁移 019）
- **启动失败清理心跳**：`--help`/`--version` 不再误启动完整服务；端口被占用启动失败时清理本实例心跳，不遗留僵尸离线节点
- **前端体验优化**：模板操作列新增「预览」按钮（只读渲染）；操作下拉按钮统一主题色系；侧边栏收缩/展开加过渡动画；长列表格列加 tooltip 截断（含农历触发列）；任务搜索支持农历关键词；发送日志跳转目标行居中闪烁定位；左下角版本号换行完整显示；用户管理「停用」统一为「禁用」
...
- **操作审计页体验**：新增「刷新」按钮，加载失败给出错误提示（此前静默）
...
- **发送日志 CSV 导出扩充**：导出列新增 `content`（发送内容）/ `request`（请求）/ `response`（响应）/ `retry_count`（重试次数），管理员可一键导出完整投递记录（共 16 列，含任务/渠道名）
- **发送日志列表列精简**：删除「触发 / 触发 IP / 错误信息 / 重试」4 个低价值列，表格从 12 列减到 8 列（展开/ID/标题/任务/渠道/状态/时间/操作），消除横向滚动、时间列完整可见；被删信息保留在展开行（新增「重试次数」）与详情页，零信息丢失
- **表格固定列背景统一**：固定列（操作列）表头改用不透明卡片色，与行单元格同色，缩窗横向滚动时不再出现半透明表头重叠
- **修复：日志详情页** 补 `watch` 导入，修复运行时 `watch is not defined` 导致的详情页渲染异常
