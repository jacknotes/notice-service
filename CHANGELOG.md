...
- 路由/接口：新增 `GET /api/audit`、`POST /api/tasks/preview`、`POST /api/auth/2fa/{setup,enable,disable}`、`POST /api/auth/2fa/verify`；`/api/auth/me` 返回完整用户信息（含 totp_enabled）；`/api/logs` 支持 sort_by/sort_order
- 以下为历史已实现项：
