...
- **前端**：Vue3 + Element Plus 深色主题（"Signal Relay" 设计），PC/移动端响应式；侧边栏可收缩/展开（状态记忆）；列表支持排序与分页，渠道/模板/任务支持「复制」，模板/任务编辑实时预览，用户管理支持显示名/邮箱、角色调整、禁用/启用与管理员强制 2FA；界面中英文双语可切换（vue-i18n，顶栏 + 个人设置双入口，默认中文）
...
make vet      # 静态检查
make fmt      # 格式化 Go 代码
cd web && npm run test   # 前端 Vitest 测试
```
...
1. **多管理员 / 普通用户**：任一管理员登录后，在「用户管理」对该用户点「重置密码」，生成一次性令牌（15 分钟有效、用完即焚），线下转交；该用户到登录页点「忘记密码」，输入用户名 + 令牌 + 新密码即可自助重置。（内置 `admin` 账号不可由此方式重置，需用第 2 条离线方式。）
